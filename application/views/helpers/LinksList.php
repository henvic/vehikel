...
class Ml_View_Helper_linksList extends Zend_View_Helper_Abstract
{
     ...
     * @param boolean $ordered ordered/unordered list; default: unordered
     * @param array   $attribs Attributes for the ol/ul tag.
     * @return string The list XHTML.
     */
    public function linksList ($linklist, $ordered = false, $attribs = false, 
    $escape = false)
    {
        $li = Array();
        if (is_array($linklist))
            foreach ($linklist as $key => $value) {
                $li[] = '<a href="' .
                 $this->view->url(array('controller' => $value), 'default') .
                 '">' . "$key</a>";
            }
        else
            foreach ($linklist as $key) {
                $li[] = '<a href="' .
                 $this->view->url(array('controller' => $key), 'default') .
                 '">' . "$key</a>";
            }
